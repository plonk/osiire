...
module ItemNamingScreen
  ...
  def complete(name, candidate, item_type)
    list = Item::ITEMS.select { |rec|
      rec[:type] == item_type && rec[:abbrev]&.start_with?(name)
    }.map { |rec| rec[:abbrev] }.sort

    if candidate
      idx = list.index(candidate)
      if idx
  ...
  def run(item_type, default_name = nil)
    name = default_name || ""
    candidate = nil
    layer_index = 0
    old_layer = nil
    ...
    field = Curses::Window.new(3, 10, tm+0, (Curses.cols - 10)/2) # lines, cols, y, x
    field.rounded_box
    # field.setpos(0, 1)
    # field.addstr("なまえ？")
    update_name = proc do
      if candidate
        s = candidate + "　" * (4 - candidate.size)
        field.setpos(1,1)
        field.attron(Curses::A_BOLD)
        field.addstr(s)
        field.attroff(Curses::A_BOLD)
        field.refresh
      else
        s = name + "＊" * (4 - name.size )
        field.setpos(1, 1)
    ...
    handle_input = proc do |c|
      ...
      when 10 # Enter
        if y == 0
          ...
          when "おぎなう"
            candidate = complete(name, candidate, item_type)
            unless candidate
              Curses.beep
            end
          when "けす"
...
if __FILE__ == $0
  ...
  name = ItemNamingScreen.run(:herb)
  Curses.close_screen
  p name
end
